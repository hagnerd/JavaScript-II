...
// Problem 1
// Get all of the email addresses so that you can email the schedule to them.
let allEmails = runners.map(runner => runner.email);
console.log(allEmails);

// Problem 2
// Get the average donation for a runner
let averageDonation =
  runners.reduce((total, { donation }) => total + donation, 0) / runners.length;
console.log(averageDonation);

// Problem 3
// Get the name of each company, ordered from largest donation to smallest.
let companiesOrderedByDontations = runners
  .sort((a, b) => (a.donation < b.donation ? -1 : 0))
  .map(runner => runner.company_name);
console.log(companiesOrderedByDontations);
